...
export default function DashboardPage() {
  ...
  const has3D = (building3DData && building3DData.summary.successful > 0) || Boolean(conversionResult?.success);

  return (
    <div className="h-full overflow-y-auto p-6 space-y-8 max-w-6xl mx-auto">
      {/* 1. Primary Hero Section: OSM -> 3D Workspace */}
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-3 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-cyan-500/30 bg-cyan-950/40 text-xs font-mono text-cyan-300">
              ...
              <span className="text-slate-300">OSM &rarr; 3D Cadastre</span>
            </div>

            <h1 className="text-3xl font-bold tracking-tight text-white sm:text-4xl">
              OSM &rarr; 3D Workspace
            </h1>
            <p className="text-sm text-slate-300 leading-relaxed">
              Convert OpenStreetMap, OSM.PBF, and GeoJSON building footprints into interactive,
              watertight 3D architectural city models with instant GLB/glTF export and cadastral intelligence.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 shrink-0">
            <Link
              href="/data"
              className="inline-flex items-center gap-2 text-sm font-semibold text-white bg-cyan-600 hover:bg-cyan-500 px-5 py-3 rounded-xl transition-all shadow-lg shadow-cyan-950/50 hover:shadow-cyan-900/60"
            >
              ...
              <span>Import OSM</span>
            </Link>

            <Link
              href="/workspace/3d"
              className="inline-flex items-center gap-2 text-sm font-medium text-slate-200 hover:text-white bg-slate-800/80 hover:bg-slate-700/80 border border-slate-700 px-5 py-3 rounded-xl transition-colors"
            >
      ...
      <div className="rounded-xl border border-slate-800 bg-[#0F172A]/60 p-6 backdrop-blur">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-800/80">
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/workspace/3d"
              className="inline-flex items-center gap-2 text-xs font-semibold text-cyan-300 hover:text-white bg-cyan-950/60 hover:bg-cyan-900 border border-cyan-500/40 px-4 py-2.5 rounded-lg transition-colors"
            >
              <span>Open 3D Workspace &rarr;</span>
            </Link>

            <a
              href="http://localhost:8000/api/v1/export/glb/latest"
              download="city_model_3d.glb"
              className="inline-flex items-center gap-1.5 text-xs font-medium text-slate-300 hover:text-white bg-slate-800 hover:bg-slate-700 border border-slate-700 px-3.5 py-2.5 rounded-lg transition-colors"
            >
      ...
      {/* 3. Available Projects Directory */}
          ...
          <div className="rounded-xl border border-cyan-500/30 bg-slate-900/50 p-5 hover:border-cyan-500/50 transition-colors">
            <div className="flex items-start justify-between">
              <div>
                ...
                <h3 className="text-base font-bold text-white">Delhi Test Area</h3>
                <p className="text-xs text-slate-400 mt-1">
                  155 building footprints from OpenStreetMap (Tagore Garden, New Delhi).
                </p>
              </div>
            ...
            <div className="mt-4 pt-3 border-t border-slate-800/80 flex items-center justify-between text-xs font-mono text-slate-400">
              <span>155 Buildings &middot; GLB Generated</span>
